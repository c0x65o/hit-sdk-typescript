 ...
 */
function getUrlFromYaml(serviceName: string): string | null {
  if (!isNode) {
    return null;
  }
 ...
 */
function parseYaml(content: string): { modules?: Array<{ name?: string; port?: number }> } | null {
  try {
    ...
    const modules: Array<{ name?: string; port?: number }> = [];
    const lines = content.split('\n');
    let inModules = false;
    let currentModule: { name?: string; port?: number } | null = null;
    let indentLevel = 0;
...
export function getWebSocketUrl(serviceName: string): string {
  ...
  const httpUrl = getServiceUrl(serviceName);
